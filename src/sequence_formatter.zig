...
fn collect(
    ...
    if (first_file == last_file) {
        try str_out.append_number(u16, first_file, null, null);
    } else {
        try str_out.append_number(u16, first_file, null, null);
        str_out.append_char(':');
        try str_out.append_number(u16, last_file, null, null);
    }
    if (buffer_isolated_files_i > buffer_isolated_files.len) {
        ...
        while (k < buffer_isolated_files_i) : (k += 1) {
            try str_out.append_number(u16, buffer_isolated_files[k], null, null);
            str_out.append_char(',');
        }
        str_out.trim_str(1);
    }
